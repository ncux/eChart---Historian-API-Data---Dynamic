...
// listen for form submit and call the function buildQueryUrlAndPlotChart, which appends form values to the dataUrl
plotButton.addEventListener('click', buildQueryUrlAndPlotChart);

function buildQueryUrlAndPlotChart(e) {
    e.preventDefault();
        ...
        xhr.onload = async () => {
            if(xhr.status === 200) {
                ...
                timeStampsAndValues.forEach(value => {
                    ...
                    valuesArray.push((parseInt(value.Value)).toFixed(0));   // removes decimal fractions
                    plotChart();
                })
            }
        };
